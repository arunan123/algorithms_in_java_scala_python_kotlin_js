package org.algorithmdb.datastructures.tree;

import java.util.concurrent.ArrayBlockingQueue;

public class BinaryTree<K extends Comparable,V> implements Tree<K,V> {
	...
	private K findMaxKey(Node node) {
		...
		if(node != null) {
			...
			if (node.getLeftNode() != null) {
				leftNode = (K) findMaxKey(node.getLeftNode());
			}

			K rightNode = null;

			if(node.getRightNode() != null) {
				rightNode = (K) findMaxKey(node.getRightNode());
			}
			if (leftNode != null) {
