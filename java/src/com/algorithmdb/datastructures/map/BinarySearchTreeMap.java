...
public class BinarySearchTreeMap<K extends Comparable<K> , V> implements OrderedMap<K, V>{
	...
	public V get(K key, Node node) {
		...
		if (cmp <0) {
			return get(key, node.getLeftNode());
		} else if (cmp > 0) {
			return get(key, node.getRightNode());
		}
	...
	private K findMaxKey(Node node) {
		...
		return findMaxKey(node.getRightNode());
	}
	...
	public K searchInBinaryTree(Node node, K key) {
		...
		if (node.getLeftNode() != null) {
			return searchInBinaryTree(node.getLeftNode(), key);
		}
		if (node.getRightNode() != null) {
			return searchInBinaryTree(node.getRightNode(), key);
		}
		return null;
	}
